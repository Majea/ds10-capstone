...
Hyphenation happens when words are cut in the middle to achieve correct line alignment. The number of hyphens appearing in the end of lines is 0. So this doesn't happen in the data sets we are treating.

### Eszett or double-s (German language)

In German, the letter 'ß' can also sometimes be written 'ss', especially when the 'ß' letter is not available. However, words written both ways don't always have the same meaning. So "bußen" and "bussen" are not the same words. In order to keep this possible difference of meaning, words written with 'ß' or 'ss' are kept separate, even if that means that the word gets a lower ranking than it should because of substitutions. 

### Abbreviations

Actually, abbreviations do not seem to be a problem in this use case. As long as they are used frequently enough, they are valid candidates for prediction.
...
During application execution, it's important to find quickly the entries that starts with the last words the user typed. This can be done by creating a hash tables with the entries we just computed. In the case of word pairs entries, the key of the hash table would be the first word and the values would be the 3 second words with highest frequency for that key. 

Example 1: if we have "the city", "the best", "the same", "the first" with respective frequencies 1, 5, 3 and 4, we would have the entry "the" associated with values ["best", "same", "first"] in the hash table. The pair "the city" would be dropped.

Example 2: Let's assume we kept the following pairs: "the train", "the car", "the guy", "my wife", "my poney" and "my children". Then the hash table would contain 2 entries. The first entry would be "the" associated with value ["train, "car, "guy"] and the second entry would be "my" associated with value ["wife", "poney", "children"]. 

In the case of 3 words series, we would have 2 levels of hash tables instead of 1 and the last level would contain the 3 suggestions. In the case of 4 words series, we would have 3 levels of hash tables and the last level would contain the 3 suggestions. If we generalize with series of n words, we would have n - 1 levels of hash tables and the last level would contain the 3 suggestions.

Example 3: the first level contains a hash table in which the key is the first word of the triplets and the values are second level hash tables. The second level hash tables have 2nd words of triplets as key and the 3rd words of the 3 series with highest frequency (starting with the keys of parent and current hash table) as values.

This should enable very fast lookup of the suggestions in the application by looking for the last words the user typed in the keys of the hash tables. The suggestions to show would be the corresponding values. 

In the case the user types n words and either we don't have analyzed series of n words or there is no such entry in that analysis, we fall back by looking into the (n-1) words analysis using the last (n-1) words until we find something we can predict. The level 0 (0 words) just shows the 3 most frequent words found in the corpus.
